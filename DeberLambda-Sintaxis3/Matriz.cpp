...
// Suma recursiva de matrices usando una lambda anidada
template <typename T>
void Matriz<T>::sumarMatricesRecursivamente(const Matriz<T>& otraMatriz, Matriz<T>& resultado) {
    // Lambda exterior que define e invoca una lambda recursiva
    auto sumaRecursiva = [&](int fila, int columna) {
        ...
        // Llamada inicial a la lambda recursiva
        recursiva(fila, columna);
    };

    // Llamada inicial a la lambda exterior
    sumaRecursiva(0, 0);
}
